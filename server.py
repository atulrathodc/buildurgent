#!/usr/bin/env python3
"""Root entry point: delegates to the BuildUrgent server in ``backend/server.py``.

Keeps the historical ``python3 server.py`` invocation working while the actual
implementation (static company site + JSON API) lives in one place. No dependencies.
