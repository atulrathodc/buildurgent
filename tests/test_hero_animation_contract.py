...
* the copy is real DOM text that reads "buildurgent. we build the software your
  business cannot wait for." and its ``aria-label`` matches it exactly, still as
  the same centred three-line block,
...
#: The copy the user reads, exactly as authored (BuildUrgent company hero).
VISIBLE_COPY = "buildurgent. we build the software your business cannot wait for."
...
def test_headline_reads_the_buildurgent_brand_line():
    """The settled, visible copy must still be the BuildUrgent brand line."""
...
def test_headline_keeps_the_centred_three_line_block():
    """The authored line structure ("buildurgent ." / "we build the software" /
    "your business cannot wait for.") survives.
